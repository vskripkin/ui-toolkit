...
import is_integer from './utils/isInteger.js';
...
Dropdown.setupFilter = function (_xInput, _oDropdown, _fFilter, _iTimeout, _fOnValue)
{
	var iTimeout = _iTimeout || 250,
	...
	if (_xInput && _xInput.get)
	{
		nInput = _xInput.get(0);
	}
	if (is_string(_xInput))
	{
		nInput = document.querySelector(_xInput);
	}
	else
	{
		nInput = _xInput;
	...
	nInput.addEventListener('keydown', function (e)
	{
		...
		else if (e.keyCode === 38 || e.keyCode === 40)
		{
			...
			if (_oDropdown.isOpened())
			{
				...
				if (is_integer(iNewSelected) && iLastSelected !== iNewSelected)
				{
	...
	_oDropdown.dropdown.addEventListener('click', function (e)
	{
		var nTarget = e.target;

		if (!nTarget.classList.contains(Class.option))
		{
			nTarget = nTarget.closest('.' + Class.option);

			if (!nTarget || !_oDropdown.dropdown.contains(nTarget)) return;
		}


		var iIndex = nTarget.index,
			sValue = _oDropdown.getValue(iIndex);

		if (nInput.tagName.toLowerCase() === 'input')
		{
			nInput.value = sValue;
		}
		else
		{
			nInput.innerHTML = sValue;
		}

		_oDropdown.hide();

		if (typeof _fOnValue !== 'undefined') _fOnValue.call(_oDropdown, nInput, sValue);
	}, false);

	_oDropdown.dropdown.style.maxHegith = '400px';
};
Dropdown.setupDropdown = function (_xInput, _oDropdown, _fOnValue)
{
	var nInput;
	...
	_oDropdown.dropdown.addEventListener('click', function (e)
	{
		...
		if (!nTarget.classList.contains(Class.option))
		{
			...
			if (!nTarget || !_oDropdown.dropdown.contains(nTarget))
			{
				return;
			}
		}
		...
		if (typeof _fOnValue !== 'undefined') _fOnValue.call(_oDropdown, nInput, nTarget, sValue);
	}, false);
...
Dropdown.prototype =
{
	...
	init: function (_options)
	{
		...
		{
			if (is_string(this.options.copyAttr)) this.options.copyAttr = this.options.copyAttr.split(' ');
	...
	addClickHandler: function (_fHandler)
	{
		...
		this.dropdown.addEventListener('click', function (e)
		{
			...
			if (!nTarget.classList.contains(Class.option))
			{
				...
				if (!nTarget || !this.dropdown.contains(nTarget))
				{
					return;
				}
			}
	...
	update: function (_xNewSelected)
	{
		...
		if (bNeedHtmlUpdate)
		{
			if (!this.currHTML)
			{
				this.currHTML = _parseHTMLColl.call(this, is_array(options.items) ? options.items : options.items.childNodes);
			...
			forEach(this.$.options, function (nElem, i)
			{
				nElem.index = i;
				aFiltered[i] = true;
			});
		...
		if (typeof this.method !== 'undefined' && is_string(this.$.query)) this[this.method](this.$.query);
	...
	show: function (_xNewSelected)
	{
		...
		{
			this.placer = new Placer(null, this.options.placeTo);
		}
	...
	filter: function (_sText)
	{
		...
		forEach(anOptions, function (nOption, i)
		{
		...
		{
			var oSelectbox = this.options.items._selectbox;
...
var _parseHTMLColl = function ( nColl, _sClass )
	...
	_toggleOptions = function (_xSelected)
	{
		if (is_integer(_xSelected))
		{
			return _toggleOption.call(this, _xSelected);
		}

		if (is_array(_xSelected))
		{
			var aResult = [],
				iNum = _xSelected.length,
				i;
